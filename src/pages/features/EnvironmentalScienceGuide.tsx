...
import {
  ...
  getEcosystemTypes,
  getGradeLevels,
  type RegionalClimateImpact,
  type SustainabilityProject,
  type EcosystemInfo,
  type EnvironmentalStandard,
  type SustainabilityAssessment,
  type ActionPlan,
} from '../../utils/environmentalUtils'

const ENV_GUIDE_SLUG = 'environmental-science-guide'
...
const EnvironmentalScienceGuide = () => {
  ...
  const handleLoadClimateImpact = async () => {
    ...
    try {
      const response = await chatbotApi.executeCapability(ENV_GUIDE_SLUG, 'global_climate_education', {
        input: ' ',
        input_type: 'text',
        parameters: { grade_level: gradeLevel, region: selectedRegion, select_region: climateRegion },
      })
      setClimateImpact(response.result as RegionalClimateImpact)
      toast.success('Climate impact loaded')
    } catch (e: any) {
      const msg = e?.payload?.detail ?? e?.message ?? 'Failed to load climate impact'
      toast.error(msg)
  ...
  const handleLoadProjects = async () => {
    setIsGenerating(true)
    try {
      const response = await chatbotApi.executeCapability(ENV_GUIDE_SLUG, 'sustainability_projects', {
        input: ' ',
        input_type: 'text',
        parameters: { grade_level: gradeLevel, region: selectedRegion, project_category: projectCategory },
      })
      const list = (response.result?.projects ?? []) as SustainabilityProject[]
      setSustainabilityProjects(list)
      toast.success(list.length ? 'Projects loaded' : 'No projects returned')
    } catch (e: any) {
      const msg = e?.payload?.detail ?? e?.message ?? 'Failed to load projects'
      toast.error(msg)
      if (e?.status === 403) toast.info('Upgrade to Premium to use this feature', { duration: 5000 })
    } finally {
      setIsGenerating(false)
  ...
  const handleLoadEcosystemInfo = async () => {
    setIsGenerating(true)
    try {
      const response = await chatbotApi.executeCapability(ENV_GUIDE_SLUG, 'ecological_systems', {
        input: ' ',
        input_type: 'text',
        parameters: { grade_level: gradeLevel, region: selectedRegion, ecosystem_type: ecosystemType },
      })
      setEcosystemInfo(response.result as EcosystemInfo)
      toast.success('Ecosystem info loaded')
    } catch (e: any) {
      const msg = e?.payload?.detail ?? e?.message ?? 'Failed to load ecosystem info'
      toast.error(msg)
      if (e?.status === 403) toast.info('Upgrade to Premium to use this feature', { duration: 5000 })
    } finally {
      setIsGenerating(false)
  ...
  const handleLoadRegionalAnalysis = async () => {
    setIsGenerating(true)
    try {
      const response = await chatbotApi.executeCapability(ENV_GUIDE_SLUG, 'regional_climate_analysis', {
        input: ' ',
        input_type: 'text',
        parameters: { grade_level: gradeLevel, region: selectedRegion, select_region: selectedRegion },
      })
      setRegionalAnalysis(response.result as RegionalClimateImpact)
      toast.success('Regional analysis loaded')
    } catch (e: any) {
      const msg = e?.payload?.detail ?? e?.message ?? 'Failed to load regional analysis'
      toast.error(msg)
      if (e?.status === 403) toast.info('Upgrade to Premium to use this feature', { duration: 5000 })
    } finally {
      setIsGenerating(false)
  ...
  const handleLoadStandards = async () => {
    setIsGenerating(true)
    try {
      const response = await chatbotApi.executeCapability(ENV_GUIDE_SLUG, 'environmental_standards', {
        input: ' ',
        input_type: 'text',
        parameters: { grade_level: gradeLevel, region: selectedRegion },
      })
      const list = (response.result?.standards ?? []) as EnvironmentalStandard[]
      setEnvironmentalStandards(list)
  ...
  const handleGenerateAssessment = async () => {
    setIsGenerating(true)
    try {
      const response = await chatbotApi.executeCapability(ENV_GUIDE_SLUG, 'sustainability_assessment_tools', {
        input: ' ',
        input_type: 'text',
        parameters: { grade_level: gradeLevel, region: selectedRegion, assessment_category: assessmentCategory },
      })
      setSustainabilityAssessment(response.result as SustainabilityAssessment)
  ...
  const handleGenerateActionPlan = async () => {
    if (!actionGoal.trim()) {
      toast.error('Enter an action goal')
      return
    }
    setIsGenerating(true)
    try {
      const response = await chatbotApi.executeCapability(ENV_GUIDE_SLUG, 'environmental_action_planning', {
        input: actionGoal.trim(),
        input_type: 'text',
        parameters: {
      ...
      setActionPlan(response.result as ActionPlan)
      toast.success('Action plan generated')
    } catch (e: any) {
      const msg = e?.payload?.detail ?? e?.message ?? 'Failed to generate action plan'
      toast.error(msg)
      if (e?.status === 403) toast.info('Upgrade to Premium to use this feature', { duration: 5000 })
    } finally {
      setIsGenerating(false)
