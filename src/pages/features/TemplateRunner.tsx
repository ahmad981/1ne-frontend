...
import { TemplateExecuteResponse, TemplateResponse } from '../../api/types'
...
const TemplateRunner = () => {
  ...
  const schemaFields = useMemo<TemplateField[]>(() => {
    ...
    const fields = (template.inputSchema as { fields?: TemplateField[] }).fields
    if (!Array.isArray(fields)) return []
    return fields.filter((field): field is TemplateField => Boolean(field?.name && field?.type))
  }, [template])
  ...
  const streamText = (text: string, section: string, onComplete: () => void) => {
  ...
  const formatOutputForCopy = (output: Record<string, any>, templateSlug?: string): string => {
    ...
    if (templateSlug === 'general-lesson-plan') {
      ...
      const formatLessonFlowSteps = (flow: any): string => {
