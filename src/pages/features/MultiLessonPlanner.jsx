...
const MultiLessonPlanner = () => {
  ...
  const [output, setOutput] = useState(sampleUnitPlan)
  ...
  const generateUnitPlan = () => {
    ...
    setTimeout(() => {
      const totalWeeks = inputs.duration_weeks

      const mockSequence = Array.from({ length: totalWeeks }).map((_, index) => ({
        week: index + 1,
        focus:
          index === 0
            ? 'Launch & Foundations'
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleInputChange('focus_area', e.target.value)
                  }
                  className="input-field"
                  required
                >
